...
export function pdf(company, contractor, items, options) {
  ...
  return {
    ...defaultDocDef,
    content: [
      ...header(company, 'ВИДАТКОВА НАКЛАДНА', docNumber, formatDateLong(docDate)),
      partiesRow(company, contractor),
      {
        table: {
          headerRows: 1,
          widths: [22, '*', 35, 40, 58, 28, 48, 62],
          body: [tableHeader(['№', 'Найменування товару', 'К-сть', 'Од.', 'Ціна', 'ПДВ%', 'ПДВ', 'Сума']), ...rows],
        },
        layout: tableLayout,
      },
      ...totalsBlock(subtotal, vatAmount, total),
      notes ? { text: `Примітка: ${notes}`, margin: [0, 10, 0, 0], fontSize: 8, color: '#999', italics: true } : {},
      ...signatures(company, contractor, 'Від постачальника:', 'Отримав:'),
      ...footer(company, 'WB', docNumber, docDate, total, contractor.edrpou),
    ],
  }
}
