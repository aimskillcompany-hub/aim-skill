...
export function pdf(company, contractor, items, options) {
  ...
  return {
    ...
    content: [
      { text: `Видаткова накладна №${docNumber} від ${formatDateLong(docDate)}`, style: 'header' },
        ...
        columns: [
          {
            ...
            stack: [
              ...
              { text: company.shortName || company.name, bold: true, fontSize: 11 },
              company.edrpou ? { text: `ЄДРПОУ: ${company.edrpou}`, style: 'small' } : {},
              company.address ? { text: company.address, style: 'small' } : {},
              company.iban ? { text: `р/р ${company.iban}`, style: 'small' } : {},
            ],
          },
          {
            width: '50%',
            stack: [
              ...
              { text: contractor.short_name || contractor.name || '—', bold: true, fontSize: 11 },
              contractor.edrpou ? { text: `ЄДРПОУ: ${contractor.edrpou}`, style: 'small' } : {},
              contractor.legal_address || contractor.address ? { text: contractor.legal_address || contractor.address, style: 'small' } : {},
              contractor.iban ? { text: `р/р ${contractor.iban}`, style: 'small' } : {},
            ],
          },
        ],
      },
      ...
      // Таблиця
      {
        table: {
          ...
          body: [
            ['№', 'Найменування товару', 'К-сть', 'Од.', 'Ціна', 'ПДВ%', 'ПДВ', 'Сума'].map(t => ({ text: t, style: 'th' })),
            ...items.map((it, i) => {
        ...
        columns: [
          {
            width: '50%',
            stack: [
              { text: 'Від постачальника:', style: 'label' },
              { text: '', margin: [0, 15] },
          ...
          },
          {
            width: '50%',
            stack: [
              { text: 'Отримав:', style: 'label' },
              { text: '', margin: [0, 15] },
              { text: '________________ ________________', fontSize: 9 },
            ],
          },
        ],
      },
