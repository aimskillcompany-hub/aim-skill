...
export function pdf(company, contractor, items, options) {
  ...
  const qrData = `AIM|WB|${docNumber}|${docDate}|${total}|${company.edrpou}|${contractor.edrpou || ''}`
  ...
  return {
    ...
    content: [
      ...
      // ═══ НАЗВА ═══
      { text: 'ВИДАТКОВА НАКЛАДНА', fontSize: 7.5, letterSpacing: 4, color: G2 },
        ...
        table: {
          ...
          body: [[{
            stack: [
              ...
              invoiceStr ? { text: `Рахунок ${invoiceStr}`, fontSize: 8.5, color: G1, margin: [0, 0, 0, 2] } : {},
              deliveryBasis ? { text: `Базис поставки: ${deliveryBasis}`, fontSize: 8.5, color: G1, margin: [0, 0, 0, 2] } : {},
              deliveryAddress ? { text: `Адреса поставки: ${deliveryAddress}`, fontSize: 8.5, color: G1 } : {},
            ].filter(Boolean),
            margin: [10, 8, 10, 8],
          }]],
        },
      ...
      // ═══ ТАБЛИЦЯ ═══
      {
        table: {
          ...
          body: [
            ['№', 'Найменування товару', 'К-сть', 'Од.', 'Вартість без ПДВ', 'ПДВ', 'Сума ПДВ', 'Сума'].map(t => ({
      ...
      { text: `Кількість найменувань: ${itemCount}`, fontSize: 8, color: G2, margin: [0, 6, 0, 0] },
      ...
      {
        columns: [
          sigBlock('ВІДПУСТИВ', company.directorPosition || 'Директор', company.director || ''),
          { width: 30, text: '' },
          sigBlock('ОТРИМАВ', contractor.contact_position || '', contractor.contact_person || ''),
        ],
      },
    ],
  }
